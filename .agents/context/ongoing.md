...
9. ✅ Harden security and privacy end-to-end: validate all user inputs, enforce HTTPS/TLS verification, avoid leaking prompts or URLs in logs, and ensure temporary data and downloaded artifacts are cleaned up securely. (Security defaults enforced; added SECURITY.md checklist.)
10. Add a comprehensive automated test suite (`pytest`) covering config, API client (with mocked HTTP), workflow conversion, and I/O utilities, plus static analysis (`mypy` and `bandit` or similar).
    - ✅ Unit tests for config, API client, workflow payloads, I/O helpers, and nodes.
    - ☐ Edge cases/end-to-end flow (mocked RunPod) and serialization round-trips.
    - ☐ Integrate `make checks` into CI (ruff, mypy, pytest, bandit).
11. Add developer and user documentation: project README, architecture overview (aligned with `vision.md`), configuration reference, example workflows, and instructions for cloning/symlinking into `ComfyUI/custom_nodes/` for local testing.
    - ✅ README basics + Makefile usage; SECURITY.md.
    - ☐ Architecture overview and config reference distilled from `vision.md`.
    - ☐ Example workflow snippet and how-to install/test in ComfyUI.
